...
export default function WeddingHallMap() {
  ...
  return (
    <div>
      {/* Hall Map */}
      <Card className="p-6 bg-gradient-to-b from-yellow-50 to-orange-50 border-2 border-amber-300">
        <div className="relative w-full bg-white rounded-xl overflow-hidden border-2 border-amber-400 p-6" style={{ minHeight: '420px' }}>
          {/* Stage */}
          <div className="absolute top-0 left-0 right-0 h-16 bg-gradient-to-b from-purple-700 to-purple-600 flex items-center justify-center rounded-t-xl">
                ...
                return (
                  <button
                    ...
                    className={`w-28 h-24 rounded-xl flex flex-col items-center justify-center font-bold text-sm border-2 transition-all shadow-sm active:scale-95 ${
                      isSelected
                        ? 'ring-4 ring-amber-400 ring-offset-1 scale-105'
                        : ''
                    } ${
                      isOverflow
        ...
        {/* Legend */}
        <div className="flex gap-5 mt-4 justify-center text-sm text-gray-600 flex-wrap">
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-blue-300 inline-block"></span>מקום פנוי</span>
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-green-400 inline-block"></span>מלא</span>
          <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-red-400 inline-block"></span>חריגה</span>
        </div>
      </Card>
                    ...
                    <div key={guest.id} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          ...
                          {guest.phone && (
                            <a
                              ...
                              className="flex items-center gap-1.5 text-sm text-blue-600 mt-1.5 hover:underline"
                        ...
                        <div className="flex flex-col items-end gap-1.5">
                          {guest.relationship && (
                            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${RELATIONSHIP_COLORS[guest.relationship] || 'bg-gray-100 text-gray-600'}`}>
                              {guest.relationship}
                            </span>
                          )}
                          {guest.side && (
                      ...
                      {guest.notes && (
                        <p className="text-xs text-gray-400 mt-2 border-t border-gray-200 pt-2">{guest.notes}</p>
                      )}
                    </div>
                  ))}
                </div>
