...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!to || !templateId) {
      return Response.json({ error: 'to and templateId are required' }, { status: 400, headers: cors });
    }
    if (!ALLOWED_TEMPLATES.has(templateId)) {
      return Response.json({ error: `templateId must be one of: ${[...ALLOWED_TEMPLATES].join(', ')}` }, { status: 400, headers: cors });
    }
    ...
    if (data.actionUrl && !/^https:\/\//i.test(String(data.actionUrl))) {
      return Response.json({ error: 'actionUrl must be an https URL' }, { status: 400, headers: cors });
    }
    ...
    return Response.json({ sent: true, providerId: result.providerId }, { headers: cors });
  } catch (e) {
